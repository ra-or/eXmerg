...
export function truncateWorksheetName(name: string): string {
  ...
  if (idx !== -1) {
    ...
    const truncatedSheet = sheetPart.length > maxSheetLen ? sheetPart.slice(0, maxSheetLen) + '…' : sheetPart;
 ...
 */
export function ensureUniqueWorksheetName(name: string, existingNames: Set<string>): string {
...
export function generateWorksheetName(params: GenerateWorksheetNameParams): string {
  ...
  const rawName = sheetCountInFile <= 1 ? base : sheet ? `${base}${SEP}${sheet}` : base;
